...
# data = generate_data.get_data(NUM_SAMPLES, "line")
# data = torch.tensor(data).float()
# labels = torch.full((NUM_SAMPLES, BATCH), real_y, dtype=torch.float)  # , device=device)
        ...
        for i, data in enumerate(dataloader, 0):
            ...
            if i == len(dataloader) - 1:
                with torch.no_grad():
                    ...
                    plt.plot(dataset.x[:, 0], dataset.x[:, 1], '.', label='original')
...
if __name__ == '__main__':
    ...
    # Optimizers
    G_optimizer = torch.optim.Adam(G.parameters(), betas=(BETA_1, 0.999))
    D_optimizer = torch.optim.Adam(D.parameters(), betas=(BETA_1, 0.999))
